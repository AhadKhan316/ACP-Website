  ...
  return (
    <section className="relative w-full h-[80vh] sm:h-[70vh] md:h-[100vh] lg:h-[100vh] text-white flex items-center justify-center overflow-hidden">
      ...
      >
        ...
        <motion.button
          className="mt-4 sm:mt-6 bg-red-600 text-white px-10 sm:px-14 py-4 rounded-full font-semibold hover:bg-red-500 transition duration-300 text-xs sm:text-sm md:text-base"
      ...
      <div className="absolute bottom-2 sm:bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2 z-30">
        {slides.map((_, index) => (
          <span
            key={index}
            className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full ${index === currentSlide ? "bg-red-400" : "bg-gray-400"
              }`}
          />
        ))}
      </div>
