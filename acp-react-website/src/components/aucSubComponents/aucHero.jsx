...
const AucHero = () => {
  ...
  const buttonVariants = {
    ...
    hover: {
      scale: 1.1,
      boxShadow: "0px 0px 15px rgba(185, 28, 28, 0.7)",
  ...
  return (
    <section className="relative w-full h-[80vh] sm:h-[70vh] md:h-[100vh] lg:h-[100vh] text-white flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-gray-900/70 via-gray-900/50 to-gray-900/70 pointer-events-none z-10"></div>
      ...
      >
        ...
        >
          ...
          <span className="text-red-700 inline-block overflow-hidden whitespace-nowrap">
        ...
        >
          Celebrate the beauty and legacy of Urdu with a global community.
        </motion.p>
        <motion.button
          className="mt-4 sm:mt-6 bg-red-700 text-white px-10 sm:px-14 py-4 rounded-full font-semibold hover:bg-red-600 transition duration-300 text-xs sm:text-sm md:text-base"
      ...
      <div className="absolute bottom-2 sm:bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2 z-30">
        {slides.map((_, index) => (
          <span
            key={index}
            className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full ${index === currentSlide ? "bg-red-700" : "bg-gray-400"
              }`}
          />
        ))}
      </div>
